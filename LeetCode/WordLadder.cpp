...
int dijkstra(int src, int dest, int num, vector<vector<int> > &adj) {
	bool visit[num];
	int dist[num];
	for (int i = 0; i < num; ++i) {
		visit[i] = false;
	...
	while (!q.empty()) {
		...
		//cout << nod1.id << " " << nod1.dist << endl;
		visit[nod1.id] = true;
		for (int i = 0; i < adj[nod1.id].size(); ++i) {
			int next = adj[nod1.id][i];
			if (dist[next] > nod1.dist + 1) {
				dist[next] = nod1.dist + 1;
				nod2.id = next;
				nod2.dist = dist[next];
				q.push(nod2);
			}
...
int ladderLength(string start, string end, unordered_set<string> &dict) {
	...
	vector <vector<int> > adj(dict.size()+2, vector<int>());
	unordered_set<string>::iterator itr1, itr2;
	int id = 1, id2;
	for (itr1 = dict.begin(), id = 1; itr1!=dict.end(); ++itr1, ++id) {
		//cout << (*itr1) << " " << id << endl;
		if (isConnected(start, *itr1)) {
			adj[0].push_back(id);
		}
		if (isConnected(end, *itr1)) {
			adj[id].push_back(dict.size()+1);
			adj[dict.size()+1].push_back(id);
		}
		for (itr2 = dict.begin(), id2 = 1; itr2!=dict.end(); ++itr2, ++id2) {
			if (id2 == id) {
				continue;
			}
			if (isConnected(*itr1, *itr2)) {
	...
	if (adj[0].empty() || adj[dict.size()+1].empty()) {
		return 0;
	}
	//puts("test");
	int dist = dijkstra(0, dict.size()+1, dict.size()+2, adj);
...
int ladderLength(string start, string end, unordered_set<string> &dict) {
	...
	for (itr1 = dict.begin(), id = 1; itr1!=dict.end(); ++itr1, ++id) {
		...
		//cout << (*itr1) << endl;
		for (itr2 = itr1, ++itr2, id2 = id+1; itr2!=dict.end(); ++itr2, ++id2) {
			if (isConnected(*itr1, *itr2)) {
				adj[id].push_back(id2);
	...
	nod1.dist = 1;
	q.push(nod1);
	vector<bool> visit(dict.size()+2, false);
	visit[0] = true;
	bool isFound = false;
	while (!q.empty()) {
		nod1 = q.front();
		q.pop();
		if (nod1.id == dict.size()+1) {
			isFound = true;
			break;
		}
		for (int i = 0; i < adj[nod1.id].size(); ++i) {
			if (!visit[adj[nod1.id][i]]) {
				nod2.id = adj[nod1.id][i];
				nod2.dist = nod1.dist+1;
				q.push(nod2);
				visit[nod2.id] = true;
			}
		}
...
// Bi-BFS, and slightly improved the graph building process
// Actually, building graph of the dictionary if useful for future search, it
// is useless and time-comsuming for the current search, because during the 
// search of the shortest path, many items in the dictionary would not be 
// reached, while we considered them in the graph building process.
struct Node {
	int id;
	int dist;
}nod1, nod2;
...
int ladderLength(string start, string end, unordered_set<string> &dict) {
	...
	for (itr1 = dict.begin(), id = 1; itr1 != dict.end(); ++itr1, ++id) {
		ids[(*itr1)] = id;
		if (isConnected((*itr1), start)) {
			adj[0].push_back(id);
		}
		if (isConnected((*itr1), end)) {
			adj[dict.size()+1].push_back(id);
			adj[id].push_back(dict.size()+1);
		}
	}
	for (itr = ids.begin(); itr != ids.end(); ++itr) {
		...
		for (int i = 0; i < tmp.size(); ++i) {
			...
			for (int j = 0; j < 26; ++j) {
				tmp[i] = 'a' + j;
				auto itr1 = ids.find(tmp);
				if (itr1 != ids.end()) {
					adj[itr->second].push_back(itr1->second);
					adj[itr1->second].push_back(itr->second);
				}
			}
			tmp[i] = ch;
		}
	...
	rq.push(nod1);
	vector<bool> visit(dict.size()+2, false);
	vector<bool> rvisit(dict.size()+2, false);
	visit[0] = true;
	rvisit[dict.size()+1] = true;
	int level = 1, rlevel = 1;
	while (!q.empty() && !rq.empty()) {
		if (q.size() < rq.size()) {
			while (!q.empty() && q.front().dist == level) {
				nod1 = q.front();
				q.pop();
			...
			while (!rq.empty() && rq.front().dist == rlevel) {
				nod1 = rq.front();
				rq.pop();
			
				for (int i = 0; i < adj[nod1.id].size(); ++i) {
					if (!rvisit[adj[nod1.id][i]]) {
						nod2.id = adj[nod1.id][i];
						rvisit[nod2.id] = true;
						if (visit[nod2.id]) {
							return (nod1.dist + q.back().dist);
						}
						nod2.dist = nod1.dist + 1;
						rq.push(nod2);
					}
